...
LIGAND_STRENGTH = {
    ...
    'dien': 1.30,
    'en':   1.40,
    # phen и bipy раньше были оба 1.50 — идентичное значение делало их
    # НЕРАЗЛИЧИМЫМИ для модели (одинаковый вектор признаков при
    # одинаковом металле/степени окисления/геометрии), хотя реальные
    # λmax и цвета отличаются (см. напр. [Cu(phen)2]2+ blue-green
    # vs [Cu(bipy)2]2+ blue в обучающих данных). phen жёстче и
    # планарнее bipy, даёт немного более сильное поле — разводим их.
    'bipy': 1.48,
    'phen': 1.52,
    'dmg':  1.55,
    'NO2':  1.60,
